...
{ type: 'LevelEditor', rulesId: string, verificationLevel?: VerificationLevel }
```
...
- `LevelEditor` may edit trick levels for a single ruleset. Verification
  levels are ranked absent (0) < `JUDGE` (1) < `OFFICIAL` (2), a grant without
  a verification level may edit levels but not verify them.
...
- `verificationLevel` – `JUDGE` or `OFFICIAL`, absent while the level is
  unverified
- `verifiedBy` / `verifiedAt` – who verified the level and when, absent while
...
settings and the record shape live in `src/services/algolia.ts`, and every
trick mutation reindexes the trick it touched.
